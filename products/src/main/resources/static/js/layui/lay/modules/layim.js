...
;
layui.define(["jquery", "layer", "laytpl", "upload"],
function(i) {
    var a = "2.0.85",
    e = layui.jquery,
    t = layui.layer,
    ...
    r.prototype.config = function(i) {
        ...
        i),
        window.JSON && window.JSON.parse ? (C(i), this) : void 0
    },
    ...
    r.prototype.chat = function(i) {
        return window.JSON && window.JSON.parse ? (I(i), this) : void 0
    },
    ...
    var d = function(i) {
        ...
        ["{{# var length = 0; layui.each(" + i.item + ", function(i, data){ length++; }}", 
         '<li layim-event="chat" data-type="' + i.type + '" data-index="{{ ' + (i.index || "i") + ' }}" id="layim-' + i.type + '{{ data.id }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>', 
         "{{# }); if(length === 0){ }}", '<li class="layim-null">' + (a[i.type] || "暂无数据") + "</li>", "{{# } }}"].join("")
    },
    u = ['<div class="layui-layim-main">', 
         ...
         '<li class="layui-icon', 
         "{{# if(!d.base.isgroup){ }}", 
         " layim-hide", "{{# } else if(!d.base.isfriend) { }}", 
         " layim-this", "{{# } }}", 
    ...
    "<li>", '<ul class="layui-layim-list layim-show layim-list-group">', 
    d({
        type: "group"
    }), "</ul>", "</li>", "</ul>", '<ul class="layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layim-show{{# } }}">', "<li>", 
    '<ul class="layui-layim-list layim-show layim-list-history">', d({
        type: "history"
    ...
    "{{# if(d.base.find){ }}", '<li class="layui-icon layim-tool-find" layim-event="find" title="查找">&#xe61f;</li>', 
    "{{# } }}", "{{# if(!d.base.copyright){ }}", 
    '<li class="layui-icon layim-tool-about" layim-event="about" title="关于">&#xe60b;</li>', "{{# } }}", 
    "</ul>", '<div class="layui-layim-search"><input><label class="layui-icon" layim-event="closeSearch">&#x1007;</label></div>', 
    "</div>"].join(""),
    c = ['<ul class="layui-layim-skin">', 
         "{{# layui.each(d.skin, function(index, item){ }}", 
         '<li><img layim-event="setSkin" src="{{ item }}"></li>', 
         "{{# }); }}", 
         '<li layim-event="setSkin"><cite>默认</cite></li>', 
         "</ul>"].join(""),
    y = ['<div class="layim-chat layim-chat-{{d.data.type}}">', 
         ...
         "</div>", '<div class="layim-chat-textarea"><textarea></textarea></div>', 
    ...
    var f, p, h, v, g, b = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>', 
                            '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', 
                            "{{# if(d.mine){ }}", '<i>{{ layui.laytpl.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', 
                            "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.laytpl.date(d.timestamp) }}</i>', 
                            "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.laytpl.content(d.content||"&nbsp") }}</div>', 
                            "</li>"].join(""),
    x = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
    k = function(i, a, l) {
        return i = i || {},
        e.ajax({
            url: i.url,
    ...
    S = function(i) {
        return t.open({
            ...
            success: function(i) {
                ...
                n.length > 0 && (n.reverse(), s.html(n.join(""))),
                T()
            },
            cancel: function(i) {
                j();
    ...
    T = function() {
        ...
        f.find(".layim-list-history").on("contextmenu", "li",
        function(a) {
            var l = e(this),
    ...
    I = function(i) {
        ...
        if (a[0]) {
            ...
            1 !== s.find("li").length || r[0] || (h.css("width", "800px"), s.css("display", "inline-block")),
            r[0] || (s.append(l(x).render(n)), a.append(l(y).render(n))),
    ...
    M = function(i, a) {
        ...
        if (a) {
            ...
            return 1 === d && (h.find(".layim-chat-list").hide(), h.css("width", "600px")),
            0 === d && t.close(g),
            !1
        }
    ...
    H = function() {
        ...
        if (i.content = a.textarea.val(), "" !== i.content.replace(/\s/g, "")) {
            if (i.content.length > n) return t.msg("内容最长不能超过" + n + "个字符");
    ...
    N = function(i) {
        ...
        t = {},
        n = a.index();
        if (i.timestamp = i.timestamp || (new Date).getTime(), A(i), !h && i.content || -1 === n) {
            if (w.message[i.type + i.id]) w.message[i.type + i.id].push(i);
            else if (w.message[i.type + i.id] = [i], "friend" === i.type) {
                var s;
                layui.each(w.friend,
                function(a, e) {
                    return layui.each(e.list,
                    function(a, e) {
                        return e.id == i.id ? (e.type = "friend", e.name = e.username, w.chat.push(e), s = !0) : void 0
                    }),
                    s ? !0 : void 0
                }),
                ...
                layui.each(w.group,
                function(a, e) {
                    return e.id == i.id ? (e.type = "group", e.name = e.groupname, w.chat.push(e), o = !0) : void 0
                }),
                o || (i.name = i.groupname, w.chat.push(i))
            } else i.name = i.name || i.username || i.groupname,
            w.chat.push(i);
            return "group" === i.type && layui.each(w.group,
            function(a, e) {
                return e.id == i.id ? (t.avatar = e.avatar, !0) : void 0
            }),
            L({
                name: "收到新消息",
    ...
    A = function(i) {
        var a = layui.data("layim")[w.mine.id] || {},
        e = a.chatlog || {};
        e[i.type + i.id] ? (e[i.type + i.id].push(i), e[i.type + i.id].length > 50 && e[i.type + i.id].shift()) : e[i.type + i.id] = [i],
        a.chatlog = e,
        layui.data("layim", {
            key: w.mine.id,
            value: a
    ...
    D = function() {
        ...
        etiger.socket.getChatHistory(a.data.type, a.data.id);
    ...
    z = function(i) {
        var a, e = f.find(".layim-list-" + i.type),
        n = {};
        if (w[i.type]) if ("friend" === i.type) layui.each(w.friend,
        function(e, l) {
            return i.groupid == l.id ? (layui.each(w.friend[e].list,
            function(e, t) {
                return t.id == i.id ? a = !0 : void 0
            }), a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
                shift: 6
            }) : (w.friend[e].list = w.friend[e].list || [], n[w.friend[e].list.length] = i, i.groupIndex = e, w.friend[e].list.push(i), !0)) : void 0
        });
        else if ("group" === i.type) {
            if (layui.each(w.group,
            function(e, t) {
                return t.id == i.id ? a = !0 : void 0
    ...
    J = function(i) {
        ...
        w[i.type] && ("friend" === i.type ? layui.each(w.friend,
        function(e, t) {
            layui.each(t.list,
            function(t, l) {
                if (i.id == l.id) {
        ...
        }) : "group" === i.type && layui.each(w.group,
        function(e, t) {
            return i.id == t.id ? (a.find(">li").eq(e).remove(), w.group.splice(e, 1), 0 === w.group.length && a.html('<li class="layim-null">暂无群组</li>'), !0) : void 0
        }))
    },
    _ = function() {
        var i = E(),
    ...
    K = function() {
        ...
        function(i) {
            var e = layui.data("layim")[w.mine.id] || {},
            t = i.keyCode;
            if ("Ctrl+Enter" === e.sendHotKey) return void(i.ctrlKey && 13 === t && H());
            if (13 === t) {
    ...
    R = function() {
        var i = ["[微笑]", "[嘻嘻]", "[哈哈]", "[可爱]", "[可怜]", "[挖鼻]", "[吃惊]", "[害羞]", "[挤眼]", "[闭嘴]", "[鄙视]", 
                 ...
                 "[弱]", "[草泥马]", "[神马]", "[囧]", "[浮云]", "[给力]", "[围观]", "[威武]", "[奥特曼]", "[礼物]", "[钟]", "[话筒]", 
                 "[蜡烛]", "[蛋糕]"],
        a = {};
        return layui.each(i,
        function(i, e) {
    ...
    Y = {
        ...
        tab: function(i) {
            ...
            t = f.find(".layui-layim-tab>li");
            "number" == typeof i ? (a = i, i = t.eq(a)) : a = i.index(),
            a > 2 ? t.removeClass(s) : (Y.tab.index = a, i.addClass(s).siblings().removeClass(s)),
        ...
        search: function(i) {
            ...
            l = function(i) {
                ...
                if ("" === a) Y.tab(0 | Y.tab.index);
                else {
                    for (var l = [], n = w.friend || [], s = w.group || [], o = "", r = 0; r < n.length; r++) for (var d = 0; d < (n[r].list || []).length; d++) - 1 !== n[r].list[d].username.indexOf(a) && (n[r].list[d].type = "friend", n[r].list[d].index = r, n[r].list[d].list = d, l.push(n[r].list[d]));
                    for (var u = 0; u < s.length; u++) - 1 !== s[u].groupname.indexOf(a) && (s[u].type = "group", s[u].index = u, s[u].list = u, l.push(s[u]));
                    if (l.length > 0) for (var c = 0; c < l.length; c++) o += '<li layim-event="chat" data-type="' + l[c].type + '" data-index="' + l[c].index + '" data-list="' + l[c].list + '"><img src="' + l[c].avatar + '"><span>' + (l[c].username || l[c].groupname || "佚名") + "</span><p>" + (l[c].remark || l[c].sign || "") + "</p></li>";
        ...
        groupMembers: function(i, a) {
            ...
            i.data("down") ? n() : (l.html("&#xe619;"), i.data("down", !0), Y.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
                tips: 3,
                time: 0,
                shift: 5,
                fix: !0,
            ...
            }), e(document).off("mousedown", n).on("mousedown", n), e(window).off("resize", n).on("resize", n), i.off("mousedown", s).on("mousedown", s))
        },
        send: function() {
            H()
        },
        ...
        face: function(i, a) {
            ...
            for (var o in R) l += '<li title="' + o + '"><img src="' + R[o] + '"></li>';
        ...
        chatLog: function(i) {
            var a = E();
            return w.base.chatLog ? (t.close(Y.chatLog.index), Y.chatLog.index = t.open({
                type: 2,
                maxmin: false,
                scrollbar:false,
                ...
                content: w.base.chatLog + "&toUser=" + a.data.id + "&type=" + a.data.type
            })) : t.msg("未开启更多聊天记录")
        },
        menuHistory: function(i, a) {
            ...
            } else "all" === s && (delete l.history, layui.data("layim", {
                key: w.mine.id,
                value: l
            }), o.html(r));
            t.closeAll("tips")
